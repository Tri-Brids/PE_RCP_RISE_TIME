...
/********************************************************************************
 * NEW: Display all 50 raw samples from the last runSingleTestADC3() call.
 *      adcResults[ch][0..49] is still valid — calculateStatistics() only
 *      reads it, never overwrites it.  Call this immediately after
 *      runSingleTestADC3() before the next resetSharedArrays().
 *******************************************************************************/
void displayRawSamplesADC(void)
{
    uint16_t ch, s;

    const char* chNames[ADC0_NUM_CH] = {
        "ADC0-IN0 (SOC0)",
        "ADC0-IN1 (SOC1)",
        "ADC0-IN2 (SOC2)"
    };
    ...
    UART_writeString("\r\n");
    UART_writeString("========================================================\r\n");
    UART_writeString("  ADC  WINDOW 15 (75ns)  -  50 RAW SAMPLES             \r\n");
    UART_writeString("========================================================\r\n");
    ...
    for(ch = 0; ch < ADC0_NUM_CH; ch++)
    {
        sprintf(uartBuffer, "\r\n  --- %s ---\r\n", chNames[ch]);
        UART_writeString(uartBuffer);
        UART_writeString("  S# : Value  |  S# : Value  |  S# : Value  |  S# : Value  |  S# : Value\r\n");
        UART_writeString("  ------------|--------------|--------------|--------------|-------------\r\n");

        /*
         * 50 samples printed as 10 rows x 5 columns
         * Row r, Col c  ->  sample index = r + (c * 10)
         * e.g. row 0: s=0, s=10, s=20, s=30, s=40
         *      row 1: s=1, s=11, s=21, s=31, s=41
         *      ...
         *      row 9: s=9, s=19, s=29, s=39, s=49
         */
        for(s = 0; s < 10; s++)
        {
            sprintf(uartBuffer,
                    "  %2u : %4u  |  %2u : %4u  |  %2u : %4u  |  %2u : %4u  |  %2u : %4u\r\n",
                    s,      adcResults[ch][s],
                    s + 10, adcResults[ch][s + 10],
                    s + 20, adcResults[ch][s + 20],
                    s + 30, adcResults[ch][s + 30],
                    s + 40, adcResults[ch][s + 40]);
            UART_writeString(uartBuffer);
        }

        /* Per-channel summary line */
        UART_writeString("  ------------|--------------|--------------|--------------|-------------\r\n");
        sprintf(uartBuffer,
                "  Min:%4u  Max:%4u  Avg:%4u\r\n",
                adc0TestResults[ch][0].min,
                adc0TestResults[ch][0].max,
                adc0TestResults[ch][0].avg);
        UART_writeString(uartBuffer);
    }
...
void main(void)
{
    ...
    waitForKeyPress();
    UART_writeString("Starting sweep: ADC3 (4 channels)\r\n");
    UART_writeString("========================================================\r\n");
    ...
    /* ----------------------------------------------------------------
     * RAW SAMPLE CAPTURE at window 15
     * Run ONE dedicated test at window 15, then dump all 50 samples.
     * adcResults[] stays valid until the next resetSharedArrays() call
     * which only happens at the start of the next runSingleTestADC3().
     * ---------------------------------------------------------------- */
    UART_writeString("\r\nCapturing 50 raw samples at Window 15 (75ns)...\r\n");
    stopEPWMs();
    delayMs(5);
    setAcquisitionWindowADC0(RAW_SAMPLE_WINDOW);   /* RAW_SAMPLE_WINDOW = 15 in .h */
    delayMs(10);
    runSingleTestADC0(0);      /* fills adcResults[0..3][0..49] */
    stopEPWMs();
    displayRawSamplesADC();   /* print before next reset clears it */
