...
/********************************************************************************
 * SOC Bitmask Definitions for ADC_forceMultipleSOC
 *******************************************************************************/
#define ADC0_SOC_MASK  ((1 << 0)  | (1 << 1)   | (1 << 2))                      // SOC0, SOC1, SOC2
#define ADC1_SOC_MASK  ((1 << 3)  | (1 << 8)   | (1 << 9))                      // SOC3, SOC8, SOC9
#define ADC2_SOC_MASK  ((1 << 10) | (1 << 11))                               // SOC10, SOC11
#define ADC3_SOC_MASK  ((1 << 4)  | (1 << 5)   | (1 << 6)  | (1 << 7))          // SOC4, SOC5, SOC6, SOC7
...
/********************************************************************************
 * Multi-Channel Polling Helper
 * 
 * Forces ALL SOCs simultaneously using ADC_forceMultipleSOC, then waits for
 * each channel individually with per-channel timeout error reporting.
 * 
 * Parameters:
 *   adcBase       - ADC module base address (e.g., myADC0_BASE)
 *   socMask       - Bitmask of SOCs to trigger (e.g., ADC0_SOC_MASK)
 *   numChannels   - Number of channels being monitored
 *   sampleCounts  - Array of sample counters for each channel
 *   completeFlags - Array of completion flags for each channel
 *   errorMessages - Array of error strings for each channel
 *******************************************************************************/
static void pollAllChannels(uint32_t adcBase,
                           uint16_t socMask,
                           uint16_t numChannels,
                           volatile uint16_t* sampleCounts,
                           volatile uint16_t* completeFlags,
                           const char** errorMessages)
{
    ...
    /* Check if ANY channel needs more samples */
    for(ch = 0; ch < numChannels; ch++)
    {
        if(sampleCounts[ch] < RESULTS_BUFFER_SIZE)
        {
            needSamples = true;
            break;
        }
    ...
    if(needSamples)
    {
        /* Force ALL SOCs simultaneously for parallel conversion */
        ADC_forceMultipleSOC(adcBase, socMask);
        
        /* Wait for each channel individually with specific error reporting */
        for(ch = 0; ch < numChannels; ch++)
        {
            if(sampleCounts[ch] < RESULTS_BUFFER_SIZE)
            {
                timeout = 0;
                while(completeFlags[ch] == 0)
                {
                    if(++timeout > 1000000)
                    {
                        UART_writeString(errorMessages[ch]);
                        while(1);  /* Halt on timeout - this is test code */
                    }
                }
                completeFlags[ch] = 0;  /* Clear flag after successful wait */
            }
        }
    }
}
...
void runSingleTestADC0(uint16_t testNumber)
{
    ...
    /* Error messages for each channel */
    static const char* adc0Errors[ADC0_NUM_CH] = {
        "\r\nERROR: ADC0 SOC0 (ADCIN0) timeout!\r\n",
        "\r\nERROR: ADC0 SOC1 (ADCIN2) timeout!\r\n",
        "\r\nERROR: ADC0 SOC2 (ADCIN4) timeout!\r\n"
    };
    ...
    {
        pollAllChannels(myADC0_BASE, ADC0_SOC_MASK, ADC0_NUM_CH,
                       adc0SampleCount, adc0Complete, adc0Errors);
...
void runSingleTestADC1(uint16_t testNumber)
{
    ...
    /* Error messages for each channel */
    static const char* adc1Errors[ADC1_NUM_CH] = {
        "\r\nERROR: ADC1 SOC3 (ADCIN0) timeout!\r\n",
        "\r\nERROR: ADC1 SOC8 (ADCIN2) timeout!\r\n",
        "\r\nERROR: ADC1 SOC9 (ADCIN4) timeout!\r\n"
    };
    ...
    {
        pollAllChannels(myADC1_BASE, ADC1_SOC_MASK, ADC1_NUM_CH,
                       adc1SampleCount, adc1Complete, adc1Errors);
        DEVICE_DELAY_US(SAMPLE_DELAY_US);
    }
...
void runSingleTestADC2(uint16_t testNumber)
{
    ...
    /* Error messages for each channel */
    static const char* adc2Errors[ADC2_NUM_CH] = {
        "\r\nERROR: ADC2 SOC10 (ADCIN2) timeout!\r\n",
        "\r\nERROR: ADC2 SOC11 (ADCIN4) timeout!\r\n"
    };
    ...
    {
        pollAllChannels(myADC2_BASE, ADC2_SOC_MASK, ADC2_NUM_CH,
                       adc2SampleCount, adc2Complete, adc2Errors);
        DEVICE_DELAY_US(SAMPLE_DELAY_US);
    }
...
void runSingleTestADC3(uint16_t testNumber)
{
    ...
    /* Error messages for each channel */
    static const char* adc3Errors[ADC3_NUM_CH] = {
        "\r\nERROR: ADC3 SOC4 (ADCIN0) timeout!\r\n",
        "\r\nERROR: ADC3 SOC5 (ADCIN1) timeout!\r\n",
        "\r\nERROR: ADC3 SOC6 (ADCIN2) timeout!\r\n",
        "\r\nERROR: ADC3 SOC7 (ADCIN3) timeout!\r\n"
    };
    ...
    {
        pollAllChannels(myADC3_BASE, ADC3_SOC_MASK, ADC3_NUM_CH,
                       adc3SampleCount, adc3Complete, adc3Errors);
        DEVICE_DELAY_US(SAMPLE_DELAY_US);
    }
